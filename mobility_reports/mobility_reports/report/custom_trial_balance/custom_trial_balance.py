...
def get_net_entries_for_account(
	company,
	from_date,
	to_date,
	root_lft,
	...
	if accounts:
		...
		gl_entries = frappe.db.sql(
			"""
			select posting_date, account,
			 sum(debit - credit) as dr_cr, is_opening, fiscal_year,
				sum(debit_in_account_currency - credit_in_account_currency) as dr_cr_ac, account_currency from `tabGL Entry`
			where company=%(company)s
			{additional_conditions}
			and posting_date <= %(to_date)s and account = %(party_account)s
			and is_cancelled = 0 group by account, voucher_type, voucher_no""".format(
		...
		for gle in gl_entries:
			...
			gle[dr_or_cr] = abs(gle.dr_cr)
			gle[dr_or_cr + "_in_account_currency"] = abs(gle.dr_cr_ac)
			gle[rev_dr_or_cr] = 0
			gle[rev_dr_or_cr + "_in_account_currency"] = 0
			adjusted_gl_entries.append(gle)

		gl_entries = adjusted_gl_entries
...
def get_data(filters):
	...
	if filters.get("show_net_values_in_party_account"):
		account_type_map = frappe._dict(
			frappe.get_all(
				"Account", fields=["name", "account_type"], filters={"company": filters.company}, as_list=1
			)
		)
		for account in gl_entries_by_account.keys():
			if account_type_map.get(account) in ("Receivable", "Payable"):
				net_entries = get_net_entries_for_account(
								filters.company,
								filters.from_date,
								filters.to_date,
								min_lft,
				...
				gl_entries_by_account[account] = net_entries
